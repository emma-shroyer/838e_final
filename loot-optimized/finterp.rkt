...
(define (type-integer? av)
  (andmap (λ (v) (or (integer? (first v)) (Int? (first v)) (err? (first v))))
          (set->list av)))

(define (type-proc? av)
  (andmap (λ (v) (or (closure? (first v)) (err? (first v))))
          (set->list av)))

(define (known-lambda? av)
  (define ls (set-map (set-filter (λ (v) (closure? (first v))) av) (λ (v) (closure-f (first v)))))
